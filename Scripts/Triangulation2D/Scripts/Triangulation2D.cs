...
namespace mattatz.Triangulation2DSystem {
	...
	public class Triangulation2D {
		...
		const float kAngleMin = 0, kAngleMax = 30f;

		public Triangle2D[] Triangle2Ds { get { return T.ToArray(); } }
		...
		List<Vertex2D> V; // vertices in PSLG
		List<Segment2D> S; // segments in PSLG
		...
		public Triangulation2D (Polygon2D polygon, float angle = 20f, float threshold = 0.1f) {
			...
			PSLG = polygon;
			V = new List<Vertex2D>(PSLG.Vertices);
			S = new List<Segment2D>(PSLG.Segments);
			Triangulate (V.Select(v => v.Coordinate).ToArray(), angle, threshold);
		}
